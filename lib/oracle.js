...
// Score d'une équipe tel que renvoyé par The Odds API : en général un simple
// entier ("1"). Pour les matchs à élimination directe finis aux tirs au but,
// on tolère une éventuelle notation de pénos entre parenthèses ("1 (4)") sans
// rien casser si elle est absente. Renvoie les buts (temps réglementaire /
// prolongation) et, le cas échéant, le nombre de tirs au but réussis.
function parseTeamScore(raw) {
    const s = String(raw ?? "").trim();
    if (!s) return { goals: null, pens: null };
    const pen = s.match(/\((\d+)\)/);
    const reg = s.replace(/\([^)]*\)/, "").match(/-?\d+/);
    return {
        goals: reg ? Number(reg[0]) : null,
        pens: pen ? Number(pen[1]) : null,
    };
}
...
export async function verifyScores() {
    ...
    for (const s of completed) {
        ...
        const h = parseTeamScore(raw(s.home_team));
        const a = parseTeamScore(raw(s.away_team));
        if (h.goals == null || a.goals == null) continue;
        // Tirs au but : simplement affichés (aucune prédiction dessus), et
        // seulement quand le fournisseur les donne pour les deux équipes.
        const penalties =
            h.pens != null && a.pens != null ? `${h.pens}-${a.pens}` : null;
        const rows = await sql`
            UPDATE poulpe.predictions
            SET actual_score = ${`${h.goals}-${a.goals}`},

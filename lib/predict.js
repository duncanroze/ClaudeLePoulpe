...
// Ajustements manuels du poulpe : pour quelques matchs, l'intuition prime sur
// le score Poisson le plus probable et on force le score exact de tête. Le
// score est exprimé du point de vue de teamA (buts teamA - buts teamB) et
// réorienté automatiquement si The Odds API inverse domicile/extérieur.
const MANUAL_SCORES = [
    { teamA: "France", teamB: "Suède", score: "3-0" },
];

function forcedScore(homeFr, awayFr) {
    for (const m of MANUAL_SCORES) {
        if (homeFr === m.teamA && awayFr === m.teamB) return m.score;
        if (homeFr === m.teamB && awayFr === m.teamA) {
...
export function buildAnalysis({ homeName, awayName, homeFr, awayFr, bookmakers }) {
    ...
    // Score de tête : ajustement manuel s'il existe, sinon le plus probable
    // cohérent avec l'issue la plus probable du marché.
    const forced = forcedScore(homeFr, awayFr);
    let lead;
    if (forced) {
        const [fh, fa] = forced.split("-").map(Number);
        lead = cells.find((c) => c.h === fh && c.a === fa) || {
            h: fh,
            a: fa,
            p: matrix[fh]?.[fa] ?? 0,
        };
    } else {
        lead = cells.find((c) => outcomeOfScore(c.h, c.a) === prediction) || cells[0];
    }
    const rest = cells.filter((c) => !(c.h === lead.h && c.a === lead.a));
